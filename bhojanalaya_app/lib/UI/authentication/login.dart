...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> onloginbtnpressed(context) async {
    ...
    try {
      var response =
          await dio.post(api_url + "/api/token/", data: json.encode(data));
      if (response.statusCode == 200) {
        ...
        if (response.data != null) {
          if (response.data['user_type'] == "C") {
            await storage.setItem("user_id", response.data['user_id']);
            await storage.setItem("token", response.data['access']);
            await storage.setItem("refresh", response.data['refresh']);
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      //     ),
                      //     padding:
                      //         EdgeInsets.fromLTRB(145.0, 15.0, 145.0, 15.0),
                      //     child: Text(
                      //       'Back',
                      //       style: kTextStyle,
                      //     ),
